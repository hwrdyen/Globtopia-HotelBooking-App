...
const Search = () => {
  ...
  const handleHotelTypeChange = (
    ...
    const currentHotelType = event.target.value;

    setSelectedHotelTypes((prevHotelTypes) =>
      event.target.checked
        ? [...prevHotelTypes, currentHotelType]
        : prevHotelTypes.filter((hotelType) => hotelType !== currentHotelType)
    );
  };

  const handleFacilityChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const currentFacility = event.target.value;

    setSelectedFacilities((prevFacilities) =>
      event.target.checked
        ? [...prevFacilities, currentFacility]
        : prevFacilities.filter((facility) => facility !== currentFacility)
    );
  };
